...
} satisfies Prisma.AbonoVendedorInclude;
...
async function getRifaVendedorOrFail(rifaVendedorId: string) {
...
export async function createAbono(
  ...
  if (saldoAnterior <= 0) {
    throw new AppError('Ese vendedor no tiene deuda pendiente para abonar.', 409);
  }
  ...
  if (subCaja.caja.rifaId !== relation.rifaId) {
    throw new AppError('La subcaja seleccionada no pertenece a la rifa de este vendedor.', 409);
  }

  if (payload.valor > saldoAnterior) {
    throw new AppError(
      `El abono no puede ser mayor a la deuda actual del vendedor (${saldoAnterior}).`,
      409
    );
  }
...
export async function getReciboByCodigo(codigo: string) {
  const recibo = await prismaClient().recibo.findUnique({
    where: {
      codigoUnico: codigo,
    },
  ...
  if (!recibo) {
    throw new AppError('No existe un recibo con ese codigo.', 404);
  }
...
export async function anularAbono(
  ...
  })) as Prisma.AbonoVendedorGetPayload<{
    include: {
      subCaja: {
        include: {
          caja: {
  ...
  if (!abono) {
    throw new AppError('El abono no existe.', 404);
  }

  if (abono.anuladoAt || abono.estado === 'RECHAZADO') {
    throw new AppError('El abono ya fue anulado.', 409);
  }

  if (!abono.subCaja?.caja?.id) {
    throw new AppError('El abono no tiene subcaja valida para reversar caja.', 409);
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const updateData: Prisma.AbonoVendedorUpdateInput = {
      estado: 'RECHAZADO',
      anuladoAt: new Date(),
