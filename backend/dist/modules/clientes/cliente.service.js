...
exports.createVentaForCliente = createVentaForCliente;
exports.createPagoForClienteVenta = createPagoForClienteVenta;
exports.listPagoClienteRecibos = listPagoClienteRecibos;
exports.getPagoClienteReciboById = getPagoClienteReciboById;
exports.getPagoClienteReciboByCodigo = getPagoClienteReciboByCodigo;
...
function buildClienteInclude(scope) {
    return {
        createdByUsuario: {
            select: {
                id: true,
...
function getClienteWhereForScope(authUser, scope, search) {
...
async function listClientes(authUser, filters) {
...
async function getClienteById(id, authUser) {
    const scope = await (0, auth_scope_1.resolveVendorAccessScope)(authUser);
    ...
    if (!cliente) {
        throw new app_error_1.AppError('Cliente no encontrado.', 404);
    }
    return cliente;
}
async function createCliente(payload, authUser) {
    return prismaClient().cliente.create({
        data: {
            ...payload,
            createdByUsuarioId: authUser?.id,
        },
        include: buildClienteInclude(await (0, auth_scope_1.resolveVendorAccessScope)(authUser)),
    });
}
async function updateCliente(id, payload, authUser) {
...
async function createVentaForCliente(clienteId, payload, authUser) {
    const prisma = prismaClient();
    await getClienteById(clienteId, authUser);
    await (0, auth_scope_1.assertVendorCanAccessRifaVendedor)(authUser, payload.rifaVendedorId);
    return prisma.$transaction(async (tx) => {
        ...
        if (boletas.length !== payload.boletaIds.length) {
            throw new app_error_1.AppError('Una o varias boletas no pertenecen a la relacion seleccionada.', 409);
        }
        const notAssignable = boletas.find((item) => item.estado !== 'ASIGNADA' || item.clienteId || item.ventaId);
        if (notAssignable) {
            throw new app_error_1.AppError(`La boleta ${notAssignable.numero} no esta disponible para asignarla a un cliente.`, 409);
        }
        const total = boletas.reduce((sum, item) => sum.plus(item.precio), new prisma_client_1.Prisma.Decimal(0));
...
async function createPagoForClienteVenta(clienteId, ventaId, payload, authUser) {
    const prisma = prismaClient();
    await getClienteById(clienteId, authUser);
    return prisma.$transaction(async (tx) => {
        ...
        if (!venta || venta.clienteId !== clienteId) {
            throw new app_error_1.AppError('La venta del cliente no existe.', 404);
        }
        if (!venta.rifaVendedorId || !venta.rifaVendedor?.id) {
            throw new app_error_1.AppError('La venta no tiene relacion rifa-vendedor asociada para registrar pagos operativos.', 409);
        }
        await (0, auth_scope_1.assertVendorCanAccessRifaVendedor)(authUser, venta.rifaVendedorId);
        const saldoPendiente = Number(venta.saldoPendiente || 0);
        if (saldoPendiente <= 0) {
            throw new app_error_1.AppError('La venta ya se encuentra completamente pagada.', 409);
        }
        if (payload.monto > saldoPendiente) {
            throw new app_error_1.AppError(`El pago no puede ser mayor al saldo pendiente de la venta (${saldoPendiente}).`, 409);
        }
        ...
        if (subCaja.caja.rifaId !== venta.rifaId) {
            throw new app_error_1.AppError('La subcaja seleccionada no pertenece a la rifa de la venta.', 409);
        }
...
async function listPagoClienteRecibos(authUser) {
    const scope = await (0, auth_scope_1.resolveVendorAccessScope)(authUser);
...
async function getPagoClienteReciboById(id, authUser) {
    const recibo = await prismaClient().pagoClienteRecibo.findUnique({
        where: { id },
        include: pagoClienteReciboInclude,
    });
    if (!recibo) {
        throw new app_error_1.AppError('El recibo del cliente no existe.', 404);
    }
    if (recibo.pagoCliente.venta.rifaVendedor?.id) {
        await (0, auth_scope_1.assertVendorCanAccessRifaVendedor)(authUser, recibo.pagoCliente.venta.rifaVendedor.id);
    }
    return recibo;
}
async function getPagoClienteReciboByCodigo(codigo) {
    const recibo = await prismaClient().pagoClienteRecibo.findUnique({
        where: { codigoUnico: codigo },
        include: pagoClienteReciboInclude,
    });
    if (!recibo) {
        throw new app_error_1.AppError('No existe un recibo de cliente con ese codigo.', 404);
    }
    return recibo;
}
